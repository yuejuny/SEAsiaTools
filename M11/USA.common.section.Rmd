...
groupFun <- function(in.DT, breaks){
  ...
  return(list(DT=in.DT, cumsum.DT))
}
...
temp <- data.table(int = sample(size = 1000, x = seq(from=0.02, to = 2.0, by = 0.01),
                                replace = T),
                   value = sample(rnorm(2000, mean = 1009, sd = 129), size = 1000))
l <- groupFun(temp, SA03.breaks)
write.csv(l[['DT']], file = file.path(US.Eng.dir, data.dir, 'R.output', 
                                          paste0(event.name,'.Low-rise.Bldg.SHK.GU.distribution.by.Sa03.',today,'.csv')))
cumsum.DT <- l[['cumsum.DT']]
```
...
ggplot(cumusum.df, aes(x = bin.center, y = CumSum.contri))+
