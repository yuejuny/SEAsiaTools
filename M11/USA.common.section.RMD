...
bins <- data.table(bin.names, bin.centers)
...
low.rise.loss[SA03 == bin.edges[1], ':='(GM.level = bin.names[1])]
for(i in 2:length(bin.edges)) {
  low.rise.loss[SA03 <= bin.edges[i] & 
             SA03 > bin.edges[i-1], ':='(GM.level = bin.names[i-1])]
}
DT <- low.rise.loss[,.(GU = sum(SHK.GU.A)), keyby = GM.level]
...
ggplot(DT, aes(x = bin.center, y = cum.contri))+
  ...
  labs(title = 'Shake low rise building loss distribution', x = 'Sa 0.3 sec, g', y = 'CumuSum contribution')
```
